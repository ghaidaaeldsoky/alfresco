...
public class AlfrescoNodeService {
    ...
    try {
        ...
        if (e.getStatusCode() == HttpStatus.CONFLICT) {
            ...
            // Use global query search under root
            String id = findFolderByTermUnderRoot(folderName);
...
public String findFolderByTermUnderRoot(String folderName) {
    try {
        // Search for folderName under -root-
        String url = alfrescoBaseUrl
                + "/alfresco/api/-default-/public/alfresco/versions/1/queries/nodes"
                + "?term=" + java.net.URLEncoder.encode(folderName, java.nio.charset.StandardCharsets.UTF_8)
                + "&rootNodeId=-root-"
                + "&nodeType=cm:folder";
